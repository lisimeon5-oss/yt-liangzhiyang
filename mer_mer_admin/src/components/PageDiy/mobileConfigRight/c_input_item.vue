  ...
  computed: {
    ...
    placePlaceholder() {
      if (this.formLang === this.defaultLangCode) {
        return this.configData.place || (this.configNme === 'titleConfig' ? this.$t('pagediy.pleaseEnterTitle') : '搜索商品名称');
      }
      const lang = this.langOptions.find((item) => item.code === this.formLang);
      const langLabel = lang ? lang.label : this.formLang;
      if (this.configNme === 'titleConfig') return this.$t('pagediy.inputTitleInLang', { lang: langLabel });
      if (this.configNme === 'titleFuConfig') return this.$t('pagediy.inputSubtitleInLang', { lang: langLabel });
      if (this.configNme === 'titleRightConfig') return this.$t('pagediy.inputRightTextInLang', { lang: langLabel });
      return this.$t('pagediy.inputHintTextInLang', { lang: langLabel });
    },
  },
  created() {
  ...
  watch: {
    configObj: {
      handler(nVal, oVal) {
        ...
        this.syncPlaceVal();
        if (this.isValI18n && !this.langLoaded) {
          this.langLoaded = true;
          this.getLanguageList();
        }
      },
