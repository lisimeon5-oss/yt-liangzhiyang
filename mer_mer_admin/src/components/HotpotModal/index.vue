...
export default {
  ...
  methods: {
    ...
    btnDialog(type) {
      if (type === 0) {
        this.$modalSure(this.translateText('确认关闭？'))
          .then((_) => {
    ...
    closeModal() {
      this.$Modal.confirm({
        title: this.translateText('提示信息'),
        content: `<p>${this.translateText('未保存内容，是否在离开前放弃保存？')}</p>`,
        okText: this.translateText('确认'),
        cancelText: this.translateText('取消'),
        onOk: () => {
          this.$Modal.remove();
    ...
    // 添加网址
    addURL(index, url, name) {
      this.areaData[index].link = url;
      this.areaData[index].name = name;
    },
    /**
    ...
    linkUrl(url, name) {
      // 将链接地址存储到对应的数据项中
      this.areaData[this.itemIndex].link = url;
