<template>
  <div class="upload-container">
    ...
    >
      ...
      <el-button v-else size="mini" type="primary" v-hasPermi="['merchant:upload:file']">{{ translateText("点击上传") }}</el-button>
    </el-upload>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    handleUploadForm(param) {
      ...
      const loading = this.$loading({
        lock: true,
        text: this.translateText('上传中，请稍候...'),
      ...
      fileFileApi(formData, data)
        .then((res) => {
          ...
          this.$message.success(this.translateText('上传成功'));
        })
        .catch((res) => {
          loading.close();
        });
