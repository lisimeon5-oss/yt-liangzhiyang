...
const FORM_ZH_KEYS = {
  '姓名：': 'systemSetting.bankUserNameLabel',
  姓名: 'systemSetting.bankUserName',
  请输入姓名: 'systemSetting.pleaseEnterBankUserName',
  '开户银行：': 'systemSetting.openingBankLabel',
  开户银行: 'systemSetting.openingBank',
  请输入开户银行: 'systemSetting.pleaseEnterOpeningBank',
  '银行卡号：': 'systemSetting.bankCardNoLabel',
  银行卡号: 'systemSetting.bankCardNo',
  请输入银行卡号: 'systemSetting.pleaseEnterBankCardNo',
  '开户地址：': 'systemSetting.bankAddressLabel',
  开户地址: 'systemSetting.bankAddress',
  请输入开户地址: 'systemSetting.pleaseEnterBankAddress',
  请输入真实姓名: 'systemSetting.pleaseEnterRealName',
  请输入微信号: 'systemSetting.pleaseEnterWechatNo',
  请输入支付宝账号: 'systemSetting.pleaseEnterAlipayAccount',
};
...
export function translateText(text) {
  ...
  try {
    ...
    if (dyn) return dyn
    const mappedKey = FORM_ZH_KEYS[text] || FORM_ZH_KEYS[text.trim()];
    if (mappedKey) {
      const mapped = i18n.t(mappedKey);
      if (mapped && typeof mapped === 'string' && mapped !== mappedKey) return mapped;
    }
