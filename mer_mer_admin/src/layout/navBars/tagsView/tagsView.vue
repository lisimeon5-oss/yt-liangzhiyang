...
export default {
  ...
  methods: {
    ...
    // 翻译标签页标题，支持 '{{ product.titleProductList }}' 形式的 i18n 键
    getTagTitle(tag) {
      this.$i18n.locale;
      return resolveNavTitle(
        tag && tag.meta && tag.meta.title,
        tag && tag.path,
    ...
    tagsViewmoveToCurrentTag() {
      this.$nextTick(() => {
        ...
        if (liDom === liFirst) {
          ...
          // 非头/尾部
          if (liIndex === 0) beforePrevL = liFirst ? liFirst.offsetLeft - 5 : '';
          else beforePrevL = liPrevTag ? liPrevTag.offsetLeft - 5 : '';
          if (liIndex === liLength) afterNextL = liLast.offsetLeft + liLast.offsetWidth + 5;
          else afterNextL = liNextTag ? liNextTag.offsetLeft + liNextTag.offsetWidth + 5 : '';
