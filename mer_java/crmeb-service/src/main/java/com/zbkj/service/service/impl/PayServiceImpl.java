...
public class PayServiceImpl implements PayService {
    ...
    public OrderPayResultResponse payment(OrderPayRequest orderPayRequest) {
        ...
        // 货到付款
        if (order.getPayType().equals(PayConstants.PAY_TYPE_HDFK) || order.getPayPrice().compareTo(BigDecimal.ZERO) <= 0) {
            Boolean hdfkBoolean = hdfkPay(order, user);
            response.setStatus(hdfkBoolean);
            newOrderNotify(orderPayRequest, user);
            return response;
        }
...
//        integralRecord.setBalance(balance);
     ...
     * @return Boolean Boolean
     */
    private Boolean hdfkPay(Order order, User user) {
        // 用户余额扣除
        Boolean execute = transactionTemplate.execute(e -> {
            Boolean update = Boolean.TRUE;
            // 订单修改
            ...
            order.setPayTime(DateUtil.date());
            order.setStatus(OrderConstants.ORDER_STATUS_WAIT_SHIPPING);
            order.setPayType(PayConstants.PAY_TYPE_HDFK);
            order.setPayChannel(PayConstants.PAY_CHANNEL_HDFK);
            orderService.updateById(order);
            return update;
        });
        if (!execute) throw new CrmebException("余额支付订单失败");
    ...
    private void newOrderNotify(OrderPayRequest orderPayRequest, User user) {
        ...
        List<MerchantOrder> merchantOrderList = merchantOrderService.getByOrderNo(orderPayRequest.getOrderNo());
        if (ObjectUtil.isNull(merchantOrderList) || merchantOrderList.size() == 0) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "未找到订单");
        }

        Map<Integer, List<MerchantOrder>> orderDetailMap = merchantOrderList.stream().collect(Collectors.groupingBy(MerchantOrder::getMerId));

        orderDetailMap.forEach((merchantId, merchantOrders) -> {
            MerchantOrder currentMerchantOrder = merchantOrders.get(0);
            String realName = currentMerchantOrder.getRealName();
            String receiveAddress = currentMerchantOrder.getUserAddress();
            String phone = currentMerchantOrder.getUserPhone();
            Integer totalNum = currentMerchantOrder.getTotalNum();
            BigDecimal totalPrice = currentMerchantOrder.getTotalPrice();
            BigDecimal payPrice = currentMerchantOrder.getPayPrice();
            MerchantInfo merchantInfo = merchantInfoService.getByMerId(merchantId);
            String xinStr = "您有新订单：\r\n" +
                    "---------------------------------------\r\n" +
                    "                               新\r\n" +
                    "                            新新新新\r\n" +
                    "                      新新新新新新新新新\r\n" +
                    "---------------------------------------\r\n" +
                    "订单号 :  " + orderPayRequest.getOrderNo() + "\r\n" +
                    "收货人 :  " + realName + "\r\n" +
                    "收货地址 : " + receiveAddress + "\r\n" +
                    "收货人系统账户 : " + tg.getAccount() + "\r\n" +
                    "小飞机昵称 : " + tg.getTgNickname() + "\r\n" +
                    "小飞机联系号码 : " + "https://t.me/" + tg.getTgUsername() + "\r\n" +
                   // "小飞机联系号码2 : " + tg.getTgUsername() + "\r\n" +
                    "电话 : " + (phone == null ? "" : phone) + "\r\n" +
                    "下单商品数量 ：" + totalNum + "件\r\n" +
                    "订单原总额 ：" + totalPrice + "\r\n" +
                    "最终金额 ：" + payPrice;
            TeleNoticeUtil.sendMessage(merchantInfo.getNewOrderNotify(), xinStr);
        });
