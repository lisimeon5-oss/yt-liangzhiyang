...
public class LogisticsServiceImpl implements LogisticService {
     ...
     */
    @Override
    public LogisticsResultKNVo info(String expressNo, String type, String com, String phone) {
        LogisticsResultKNVo resultVo = new LogisticsResultKNVo();
        String redisKey = RedisConstants.LOGISTICS_KEY + expressNo;
        JSONObject result = getCache(redisKey);
        if (ObjectUtil.isNotNull(result)) {
            return JSONObject.toJavaObject(result, LogisticsResultKNVo.class);
        }
        ...
        if (logisticsType.equals("2")) {// 阿里云查询
            ...
            String url = StrUtil.format(SysConfigConstants.LOGISTICS_QUERY_ALIYUN_URL, expressNo,com);
           ...
           //result = data.getJSONObject("result");
            ...
            resultVo = JSONObject.toJavaObject(result, LogisticsResultKNVo.class);
        }
        return resultVo;
    }
     ...
     * 一号通vo转公共返回vo
     */
    private LogisticsResultKNVo queryToResultVo(OnePassLogisticsQueryVo queryVo) {
        LogisticsResultKNVo resultVo = new LogisticsResultKNVo();
       // resultVo.setNumber(queryVo.getNum());
        ...
        if (CollUtil.isNotEmpty(queryVo.getContent())) {
            List<LogisticsResultListKNVo> list = CollUtil.newArrayList();
            queryVo.getContent().forEach(i -> {
                LogisticsResultListKNVo listVo = new LogisticsResultListKNVo();
    ...
    private void checkResult(JSONObject data) {
        if (!data.getString("state").equals("3")) {
            throw new CrmebException(data.getString("message"));
        }
        /*
