...
import com.zbkj.common.vo.LogisticsResultKNVo;
import com.zbkj.common.vo.LogisticsResultVo;
import com.zbkj.common.vo.MyRecord;
import com.zbkj.service.dao.OrderDao;
import com.zbkj.service.service.*;
import com.zbkj.service.util.TelegramUtil;
...
public class OrderServiceImpl extends ServiceImpl<OrderDao, Order> implements OrderService {
    ...
    @Autowired
    private UserTelegramInfoService telegramInfoService;
    @Autowired
    ...
    public Boolean send(OrderSendRequest request, SystemAdmin systemAdmin) {
        ...
        if (!request.getIsSplit()) {// 全部发货
            ...
            if (request.getDeliveryType().equals(OrderConstants.ORDER_DELIVERY_TYPE_MERCHANT)) {
                return sendMerchant(request, order, merchantOrder);
            }
...
//        }
     ...
     * @param merchantOrder   商户订单
     */
    private Boolean sendMerchant(OrderSendRequest request, Order order, MerchantOrder merchantOrder) {

        String deliveryCarrier = request.getDeliveryCarrier();
        String carrierPhone = request.getCarrierPhone();
        String sendGroupId = request.getSendGroupId();
        List<OrderDetail> orderDetailList = orderDetailService.getByOrderNo(order.getOrderNo());
        List<OrderInvoiceDetail> orderInvoiceDetailList = new ArrayList<>();
        List<OrderDetail> orderDetailUpdateList = new ArrayList<>();
        orderDetailList.forEach(od -> {
...
//        }
        ...
        telegramSendOrderGroup(orderDetailList, merchantOrder, deliveryCarrier, carrierPhone, sendGroupId);
        return execute;
    }
...
//        }
     ...
     */
    @Override
    public LogisticsResultKNVo getLogisticsInfoByMerchant(Integer invoiceId, SystemAdmin systemAdmin) {
     ...
     */
    @Override
    public LogisticsResultKNVo getLogisticsInfo(Integer invoiceId) {
        OrderInvoice orderInvoice = orderInvoiceService.getById(invoiceId);
        if (ObjectUtil.isNull(orderInvoice)) {
            throw new CrmebException(OrderResultCode.ORDER_INVOICE_NOT_EXIST);
        }
...
//        temMap.put("thing7", "您的订单已发货");
    ...
    private void telegramSendOrderGroup(List<OrderDetail> orderDetailList, MerchantOrder merchantOrder, String deliveryCarrier, String carrierPhone, String sendGroupId) {
        User user = userService.getById(merchantOrder.getUid());
        Merchant merchant = merchantService.getById(merchantOrder.getMerId());
        MerchantInfo merchantInfo = merchantInfoService.getByMerId(merchantOrder.getMerId());
        UserTelegramInfo tg = telegramInfoService.getBaseMapper().selectOne(new QueryWrapper<UserTelegramInfo>().lambda().eq(UserTelegramInfo::getAccount, user.getAccount()));
        if (tg == null) throw new CrmebException("账户信息误!");
        String rmk = "";
        for (int i = 0; i < orderDetailList.size(); i++) {
            Integer sort = i + 1;
            OrderDetail orderDetail = orderDetailList.get(i);
            rmk += ("     " + sort + ". " + orderDetail.getProductName() + "  x " + orderDetail.getPayNum() + "\r\n");
        }
        String str = "######订单配送信息########\r\n" +
                "      အမှာစာပေးပို့မှုအချက်အလက်\r\n\r\n" +
                "收货人信息（ကုန်ပို့သူအချက်အလက်）：\r\n" +
                "  1.订单号(အော်ဒါနံပါတ်)：      " + merchantOrder.getOrderNo() + "\r\n" +
                "  2.收货人（ကုန်ပို့သူ）：" + merchantOrder.getRealName() + "\n" +
                "  3.地址（လိပ်စာ）：" + merchantOrder.getUserAddress() + "\n" +
                "  4.账户（အကောင့်） : " + tg.getAccount() + "\n" +
                "  5.小飞机昵称（Telegram နာမည်ပြောင်） :          " + tg.getTgNickname() + "\n" +
                "  6.小飞机联系号码（Telegram ဆက်သွယ်ရန်နံပါတ်） : \r\n" +
                "https://t.me/" + tg.getTgUsername() + "\r\n" +
                "  7.电话（တယ်လီဖုန်း）: " + (user.getPhone() == null ? "" : user.getPhone()) + "\r\n" +
                "  8.下单商品数量（အော်ဒါအရေအတွက်） ：" + merchantOrder.getTotalNum() + "件\r\n" +
                "  9.订单总额（အော်ဒါစုစုပေါင်း） ：  ฿" + merchantOrder.getPayPrice() + "\r\n" +
                "\r\n商品清单（ထုတ်ကုန်စာရင်း）:" + "\r\n" + rmk + "\n" +
                "--------------------------------------------------------------------------------\r\n" +
                "送货员（ပို့ပေးတဲ့ကောင်လေး）：" + deliveryCarrier + "\n" +
                "送货员电话（Deliveryman ဖုန်းနံပါတ်）：" + carrierPhone + "\n" +
                "收款金额（ငွေပေးချေမှုပမာဏ）：" + merchantOrder.getPayPrice();
        telegramUtil.sendMessageDelivery(merchantOrder.getOrderNo(),str, tg.getTgId(),merchant.getName(),merchantInfo.getServiceLink(), carrierPhone, sendGroupId);
    }
