...
public class LoginServiceImpl implements LoginService {
    ...
    @Autowired
    private UserTelegramInfoService userTelegramInfoService;
    ...
    //@Override
    @Transactional
    public LoginResponse autoLogin(LoginTelegramRequest loginRequest) {
        logger.info("=======WebApp登录开始：{}",loginRequest);
        // 构建DES对象
        String key = "zhajianmian88888";
        DES des = SecureUtil.des(key.getBytes());
        // 解密Base64字符串
        // String decryptBase64 = des.decryptStr(loginRequest.getAccount());

        String decryptStr = des.decryptStr(loginRequest.getAccount());
        String[] array = decryptStr.split("&");
        String tgId = array[0];
        String tgUsername = array[1];
        String tgNickname = array[2];
        //查询用户信息
        String account = "tg" + tgId;
        ...
        if (ObjectUtil.isNull(user)) {
            ...
            String md5Hex = DigestUtil.md5Hex(account.substring(3, 8));
            user.setPwd(md5Hex);
            user.setNickname(tgNickname);
            //user.setPhone(user);
            user.setLastLoginTime(new Date());
            boolean row = userService.save(user);
            if (!row) throw new CrmebException("登录失败");
            //插入小飞机信息
            UserTelegramInfo userTelegramInfo = new UserTelegramInfo();
            userTelegramInfo.setAccount(account);
            userTelegramInfo.setTgId(Long.valueOf(tgId));
            userTelegramInfo.setTgNickname(tgNickname);
            userTelegramInfo.setTgUsername(tgUsername);
            row = userTelegramInfoService.save(userTelegramInfo);
            if (!row) throw new CrmebException("登录失败!");
           ...
           /* Integer spreadPid = Optional.ofNullable(loginRequest.getSpreadPid()).orElse(0);
            ...
            // 3. 存在则直接登录
            UserTelegramInfo userTelegramInfo = userTelegramInfoService.getOne(new QueryWrapper<UserTelegramInfo>().lambda().eq(UserTelegramInfo::getAccount, account));
            if (ObjectUtil.isNull(userTelegramInfo)) {
                throw new CrmebException("飞机信息不存在!");
            }
            userTelegramInfo.setTgUsername(tgUsername);
            user.setNickname(tgNickname);
            userTelegramInfo.setTgNickname(tgNickname);
            userTelegramInfoService.updateById(userTelegramInfo);
            Integer spreadPid = Optional.ofNullable(loginRequest.getSpreadPid()).orElse(0);
            return commonLogin(user, spreadPid);
        }
    }
    ...
    @Override
    @Transactional
    public LoginResponse autoLoginMiniApp(LoginTelegramRequest loginRequest) {
        logger.info("=======MiniApp登录开始：{}",loginRequest);
        // 构建DES对象
        String tgId = loginRequest.getAccount();
        String tgUsername = loginRequest.getUsername();
        String tgNickname = loginRequest.getNickname();
        //查询用户信息
        String account = "tg" + tgId;
        User user = userService.getOne(new QueryWrapper<User>().lambda().eq(User::getAccount, account));
        // 此用户不存在，走新用户注册流程
