...
//    private RedisTemplate<String, Object> redisTemplate;
    ...
    public boolean setIfAbsent(String key, Object value, long expireTime, TimeUnit timeUnit) {
        try {
            ...
            Boolean result = getRedisTemplate().opsForValue().setIfAbsent(key, value, expireTime, timeUnit);
    ...
    /**
     * 执行 Lua 脚本（用于释放锁时保证原子性）
     *
     * @param script Lua 脚本
     * @param key    KEYS[1]
     * @param value  ARGV[1]（用于判断锁是否是自己的）
     * @return 脚本执行结果
     */
    public Long executeLuaScript(String script, String key, String value) {
        try {
            ...
            return getRedisTemplate().execute(redisScript, keys, value);
        } catch (Exception e) {
            e.printStackTrace();
            return 0L;
        }
